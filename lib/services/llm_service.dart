...
if (ReturningToRoute) {
  situationPrompt = '''
  사용자가 경로에서 약간 벗어났어
  원래 경로인 다음 단계로 돌아가도록 안내해줘
  "경로를 벗어났습니다" 와 같은 직접적인 표현 대신, 자연스럽게 방향을 수정해주는 안내를 해줘
  예: "11시 방향으로 약간 틀어서 이동하세요.", "뒤로 돌아서 다음 안내 지점까지 이동하세요."
  ''';
} else {
  situationPrompt = '''
...
''';
  ...
  final response = await http.post(
    ...
    headers: {
      'Authorization': 'Bearer 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',
...
Future<String> getNextGuideSentence({
  ...
  try {
    ...
    if (minDist > 50) {
      return '경로에서 50m 이상 벗어났습니다. 경로 재탐색이 필요합니다.';
    }

    final targetStep = steps[bestIndex];
    final bool isReturningToRoute = minDist > 15; //15m 초과 시

    return await generateLLMNavigationGuide(
      currentLocation: {'lat': lat, 'lng': lng},
      step: targetStep,
      currentAngle: angle,
      ReturningToRoute: isReturningToRoute, // 경로 복귀 필요 여부 전달
    );
  } catch (e) {
