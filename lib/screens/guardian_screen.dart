...
class _GuardianHomeScreenState extends State<GuardianHomeScreen> {
  ...
  Future<void> _checkPendingSos() async {
    ...
    if (now.difference(timestamp.toDate()).inMinutes < 10 && mounted) {
      showDialog(
        context: context,
  ...
  Future<void> _getCurrentLocation() async {
    ...
    if (status.isGranted) {
      try {
        ...
        if (mounted) _showErrorSnackBar('위치 정보를 가져오지 못했습니다.');
      }
      ...
      if (mounted) _showErrorSnackBar('위치 권한 상태: $status');
    }
  }
          ...
          actions: [
            TextButton(
              onPressed: () async {
                ...
                try {
                  final blindDoc = await FirebaseFirestore.instance
